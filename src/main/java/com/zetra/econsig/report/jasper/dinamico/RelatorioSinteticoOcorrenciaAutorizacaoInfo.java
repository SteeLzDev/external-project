...
/**
 * <p>Title: RelatorioSinteticoInfo</p>
 * <p>Description: Definição de colunas do Relatório Sintético de Consignações</p>
 ...
 * $Date$
 */
public class RelatorioSinteticoOcorrenciaAutorizacaoInfo extends DynamicReportInfo {
    /** Log object for this class. */
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(RelatorioSinteticoOcorrenciaAutorizacaoInfo.class);

    public RelatorioSinteticoOcorrenciaAutorizacaoInfo(Relatorio relatorio) {
        super(relatorio);
    }
    ...
    protected DynamicReportBuilder reportDesign(AcessoSistema responsavel) {
        ...
        try {
            ...
            if (campos != null) {
                ...
                if (campos.contains(Columns.NSE_DESCRICAO)) {
                    ColumnBuilder columnBuilder = buildColumn(ApplicationResourcesHelper.getMessage("rotulo.natureza.servico.titulo", responsavel), 70, false, "nse_descricao", String.class.getName(), borda);
                    AbstractColumn columnCor = columnBuilder.build();
                    reportBuilder.addColumn(columnCor);
                }
                if (campos.contains(Columns.CNV_COD_VERBA)) {
                    ColumnBuilder columnBuilder = buildColumn(ApplicationResourcesHelper.getMessage("rotulo.verba.singular", responsavel), 70, false, "cnv_cod_verba", String.class.getName(), borda);
                    AbstractColumn columnCor = columnBuilder.build();
                    reportBuilder.addColumn(columnCor);
                }
                ...
                if (campos.contains(Columns.OCA_DATA)) {
                    ColumnBuilder columnBuilder = buildColumn(ApplicationResourcesHelper.getMessage("rotulo.relatorio.periodo.ocorrencia", responsavel), 55, false, "oca_data", String.class.getName(), borda);
                    AbstractColumn columnCor = columnBuilder.build();
                    reportBuilder.addColumn(columnCor);
                }

                if (campos.contains(Columns.TOC_DESCRICAO)) {
                    ColumnBuilder columnBuilder = buildColumn(ApplicationResourcesHelper.getMessage("rotulo.relatorio.ocorrenciaconsignacao.tipo", responsavel), 55, false, "toc_descricao", String.class.getName(), borda);
                    AbstractColumn columnCor = columnBuilder.build();
                    reportBuilder.addColumn(columnCor);
                }

                if (campos.contains(Columns.TMO_DESCRICAO)) {
                    ColumnBuilder columnBuilder = buildColumn(ApplicationResourcesHelper.getMessage("rotulo.relatorio.ocorrenciaconsignacao.tipo.motivo.operacao", responsavel), 55, false, "tmo_descricao", String.class.getName(), borda);
                    AbstractColumn columnCor = columnBuilder.build();
                    reportBuilder.addColumn(columnCor);
                }
