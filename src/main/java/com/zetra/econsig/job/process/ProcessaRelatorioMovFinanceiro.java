...
/**
 * <p>Title: ProcessaRelatorioMovFinanceiro</p>
 ...
 * $Date$
 */
public final class ProcessaRelatorioMovFinanceiro extends ProcessaRelatorio {
    /** Log object for this class. */
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(ProcessaRelatorioMovFinanceiro.class);

    public ProcessaRelatorioMovFinanceiro(Relatorio relatorio, Map<String, String[]> parameterMap, HttpSession session, Boolean agendado, AcessoSistema responsavel) {
    ...
    protected void executar() {
        ...
        // Para esta classe temos 3 relatorio.getTipo() = mov_mes, mov_mes_cor e mov_mes_csa
        StringBuilder nome = new StringBuilder((getNomeArquivo(ApplicationResourcesHelper.getMessage("rotulo.nome.arquivo.relatorio." + relatorio.getTipo().replace("_", "."), responsavel), responsavel, parameterMap, null)));
        ...
        String matricula = getFiltroRseMatricula(parameterMap, subTitulo, nome, session, responsavel);
        String cpf = getFiltroCpf(parameterMap, subTitulo, nome, session, responsavel);
        ...
        List<String> lstPrdRealizado = getFiltroParcelaRealizado(parameterMap, subTitulo, nome, session, responsavel);
        // Se usuário csa e não agrupa, exibe por consignatária
        boolean agrupamento = getFiltroAgrupamento(parameterMap, subTitulo, nome, session, responsavel);
        String order = (responsavel.isCseSupOrg() && csaCodigo == null) ? "CONSIGNATARIA" : (agrupamento ? "ORGAO" : "CONSIGNATARIA");
        String tituloGrupo = (responsavel.isCseSupOrg() && csaCodigo == null) ? ApplicationResourcesHelper.getMessage("rotulo.consignataria.singular", responsavel).toUpperCase() : (agrupamento ? ApplicationResourcesHelper.getMessage("rotulo.orgao.singular", responsavel).toUpperCase() : ApplicationResourcesHelper.getMessage("rotulo.consignataria.singular", responsavel).toUpperCase());
