...
/**
 * <p> Title: ProcessaRelatorioAlteracaoADEValorRetorno</p>
 * <p> Description: Classe para processamento de relatorios de alteração de valor de ADE no retorno.</p>
 ...
 * $Date$
 */
public final class ProcessaRelatorioAlteracaoADEValorRetorno extends ProcessaRelatorio {
    /** Log object for this class. */
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(ProcessaRelatorioAlteracaoADEValorRetorno.class);

    public ProcessaRelatorioAlteracaoADEValorRetorno(Relatorio relatorio, Map<String, String[]> parameterMap, HttpSession session, Boolean agendado, AcessoSistema responsavel) {
    ...
    protected void executar() {
        if (!parameterMap.containsKey("periodo")) {
            ...
            String periodo = formatarPeriodo(strPeriodo);
            String titulo = relatorio.getTitulo();
            StringBuilder subtitulo = new StringBuilder(ApplicationResourcesHelper.getMessage("rotulo.periodo.singular.arg0", responsavel, strPeriodo));

            String nome = getNomeArquivo(ApplicationResourcesHelper.getMessage("rotulo.nome.arquivo.relatorio.alteracao.ade.valor.retorno", responsavel), responsavel, parameterMap, null);

            criterio.setAttribute(ReportManager.PARAM_NAME_PERIODO, periodo);
            ...
            if (!TextHelper.isNull(parameterMap.get("svcCodigo")[0])) {
                ...
                String values[];
                subtitulo.append(" - ").append(ApplicationResourcesHelper.getMessage("rotulo.servico.singular.upper.arg0", responsavel, ""));
                for (String svc : svcs) {
                    values = svc.split(";");
