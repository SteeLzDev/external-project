...
public class TermosDeUsoService extends RestService {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(TermosDeUsoService.class);
    ...
    @Secured
    @Path("/show")
    public Response consultarTermosDeUso(String tipoTermo) {
        ...
        String tocCodigo;
        String chaveAceitacaoTermoDeUso;
        if (tipoTermo.equals("salarypay")) {
            tocCodigo = CodedValues.TOC_ACEITACAO_TERMO_DE_USO_SALARYPAY;
            chaveAceitacaoTermoDeUso = CodedValues.TPC_DATA_TERMO_DE_USO_SALARYPAY_CSE;
            if (responsavel.isOrg()) {
                chaveAceitacaoTermoDeUso = CodedValues.TPC_DATA_TERMO_DE_USO_SALARYPAY_ORG;
            } else if (responsavel.isSer()) {
                chaveAceitacaoTermoDeUso = CodedValues.TPC_DATA_TERMO_DE_USO_SALARYPAY_SER;
            } else if (responsavel.isCsa()) {
                chaveAceitacaoTermoDeUso = CodedValues.TPC_DATA_TERMO_DE_USO_SALARYPAY_CSA;
            } else if (responsavel.isCor()) {
                chaveAceitacaoTermoDeUso = CodedValues.TPC_DATA_TERMO_DE_USO_SALARYPAY_COR;
            } else if (responsavel.isSup()) {
                chaveAceitacaoTermoDeUso = CodedValues.TPC_DATA_TERMO_DE_USO_SALARYPAY_SUP;
            }
        } else {
            tocCodigo = CodedValues.TOC_ACEITACAO_TERMO_DE_USO_MOBILE;
            chaveAceitacaoTermoDeUso = CodedValues.TPC_DATA_TERMO_DE_USO_CSE;
            if (responsavel.isOrg()) {
        ...
        if (!TextHelper.isNull(paramAceitacaoTermoDeUso)) {
            ...
            if (arqTermoDeUso == null) {
                return genericError(new Exception(ApplicationResourcesHelper.getMessage("mensagem.erro.interno.termo.de.uso.nao.encontrado", responsavel)));
            } else {
                try {
                    msg = FileHelper.readAll(new FileInputStream(arqTermoDeUso.getAbsolutePath()), "ISO-8859-1".intern());
    ...
    @POST
    @Secured
    @Path("/aceitarTermosUso")
    public Response aceitarTermosDeUso(String tipoTermo) {
        AcessoSistema responsavel = (AcessoSistema) securityContext.getUserPrincipal();
        tipoTermo = tipoTermo == null ? "" : tipoTermo;
		...
		return consultarTermosDeUso(tipoTermo);
    }
}
