...
/**
 * <p>Title: RegraAnoMesFimPassado</p>
 * <p>Description: Classe com a implementação MYSQL da regra com a verificação se não
 *    está enviando contratos com prazo final passado.</p>
 ...
 * $Date$
 */
public class RegraAnoMesFimPassado extends Regra {
    /** Log object for this class. */
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(RegraAnoMesFimPassado.class);
    ...
    private List<TransferObject> buscaQtdLinhasInvalidas() {
        ...
        try {
            ...
            query.append("WHERE ade_ano_mes_fim IS NOT NULL AND ade_ano_mes_fim < '").append(periodo).append("'");
    ...
    private List<TransferObject> buscaLinhasInvalidas(String operacao) {
        ...
        try {
            ...
            query.append("AND ade_ano_mes_fim IS NOT NULL AND ade_ano_mes_fim < '").append(periodo).append("'");
            if (estCodigos != null && estCodigos.size() > 0) {
                query.append("AND ").append(Columns.ORG_EST_CODIGO).append(" IN (:estCodigos) ");
                queryParams.addValue("estCodigos", estCodigos);
            }
