 ...
 * $
 */
public class GovRJ6 extends GovRJ3 {
    ...
    protected List<TransferObject> listaContratosSemMargemCandidatosv2(List<Short> marCodigos) throws ExportaMovimentoException {
        ...
        query.append("SELECT tmp.rse_codigo, tmp.ade_codigo, tmp.ade_vlr, ade.ade_data,");
        query.append("case ");
        query.append("  when x.pse_vlr = " + MARGEM_CARTAO_40 + " and (ade.ade_data < '" + DATA_INI_MUDANCA_MARGEM + "') then " + MARGEM_CARTAO_50 + " ");
        query.append("  when x.pse_vlr = " + MARGEM_CARTAO_35 + " then " + MARGEM_CARTAO_35 + " ");
        query.append("  else x.pse_vlr end AS ade_inc_margem, ");
        ...
        query.append("  when x.pse_vlr = " + MARGEM_CARTAO_40 + " and (ade.ade_data < '" + DATA_INI_MUDANCA_MARGEM + "') then " + MARGEM_CARTAO_50 + " ");
        query.append("  when x.pse_vlr = " + MARGEM_CARTAO_35 + " then " + MARGEM_CARTAO_35 + " ");
        query.append("  else " + MARGEM_CARTAO_40 + " end) ");
        query.append(") < 0.00 ");
        query.append(")) ");
                ...
                for (final TransferObject contrato : contratos) {
                    ...
                    if (!rseCodigo.equals(rseCodigoAnterior)) {
                        ...
                        for (final MargemTO margemTO : margens) {
                            ...
                            if (CodedValues.INCIDE_MARGEM_SIM_3.equals(marCodigo) || MARGEM_CARTAO_50.equals(marCodigo) || MARGEM_CARTAO_35.equals(marCodigo) || MARGEM_CARTAO_40.equals(marCodigo)) {
                    ...
                    if (margemRestante.get(adeIncMargem).signum() < 0) {
                        ...
                        if (!CodedValues.INCIDE_MARGEM_SIM_3.equals(adeIncMargem) && !MARGEM_CARTAO_50.equals(adeIncMargem) && !MARGEM_CARTAO_35.equals(adeIncMargem) && !MARGEM_CARTAO_40.equals(adeIncMargem)) {
