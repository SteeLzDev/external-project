 ...
 * <p>Description: Implementação MYSQL da regra com a comparação da média quantidade total de registros gerados
 *                 por código de verba ao longo dos últimos períodos.</p>
 ...
 * $Date$
 */
public class RegraVrcMediaTotalRegistros extends Regra {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(RegraVrcMediaTotalRegistros.class);
        ...
        if (variacaoCsa != null) {
            ...
            while (it.hasNext()) {
                ...
                if (qtdAnterior.compareTo(new BigDecimal(0)) > 0 ) {
                    ...
                    if (diferencaPercentual.compareTo(Double.valueOf(limiteErro)) >= 0) {
                    ...
                    } else if (diferencaPercentual.compareTo(Double.valueOf(limiteAviso)) >= 0) {
                        rrvValorEncontrado.append("<br>\n").append(csaNome).append(" - ").append(codVerba).append(": ").append(qtdAtual).append("/").append(qtdAnterior).append(" *");
    ...
    private List<TransferObject> buscaVariacaoPorCsa() {
        ...
        try {
            ...
            query.append(" from tb_historico_mov_fin innerhmf where innerhmf.cnv_codigo = cnv.cnv_codigo group by  cnv.cnv_cod_verba) ");
            ...
            query.append(" from tb_historico_mov_fin interhmf where interhmf.cnv_codigo = cnv.cnv_codigo group by  cnv.cnv_cod_verba) ").append(" month) group by  cnv.cnv_cod_verba), 0)) as QTD_ANTERIOR, ");
