...
/**
 * <p>Title: RegraConveniosBloqueados</p>
 * <p>Description: Classe com a implementação MYSQL da regra com a verificação se não
 *    está enviando contratos de convênios BLOQUEADAS.</p>
 ...
 * $Date$
 */
public class RegraConveniosBloqueados extends Regra {
    /** Log object for this class. */
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(RegraConveniosBloqueados.class);
    ...
    private List<TransferObject> buscaQtdLinhasInvalidas() {
        ...
        try {
            ...
            query.append(" INNER JOIN ").append(Columns.TB_CONVENIO).append(" ON (").append(Columns.AMV_CNV_CODIGO).append(" = ").append(Columns.CNV_CODIGO).append(")");
            query.append(" INNER JOIN ").append(Columns.TB_ORGAO).append(" ON (").append(Columns.CNV_ORG_CODIGO).append(" = ").append(Columns.ORG_CODIGO).append(")");
            query.append(" WHERE ").append(Columns.AMV_OPERACAO).append(" IN ('A', 'I')");
            query.append(" AND ").append(Columns.CNV_SCV_CODIGO).append(" <> ").append(CodedValues.SCV_ATIVO);
    ...
    private List<TransferObject> buscaLinhasInvalidas(String operacao) {
        ...
        try {
            ...
            query.append(" INNER JOIN ").append(Columns.TB_CONVENIO).append(" ON (").append(Columns.AMV_CNV_CODIGO).append(" = ").append(Columns.CNV_CODIGO).append(")");
            query.append(" INNER JOIN ").append(Columns.TB_ORGAO).append(" ON (").append(Columns.CNV_ORG_CODIGO).append(" = ").append(Columns.ORG_CODIGO).append(")");
            query.append(" INNER JOIN ").append(Columns.TB_CONSIGNATARIA).append(" ON (").append(Columns.CNV_CSA_CODIGO).append(" = ").append(Columns.CSA_CODIGO).append(")");
