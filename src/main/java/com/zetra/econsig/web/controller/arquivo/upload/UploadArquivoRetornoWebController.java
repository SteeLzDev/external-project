...
/**
 * <p>Title: UploadArquivoRetornoWebController</p>
 * <p>Description: Controlador Web para o caso de uso Upload Arquivo de retorno.</p>
 ...
 */
@Controller
@RequestMapping(method = { RequestMethod.POST }, value = { "/v3/uploadArquivoRetorno" })
public class UploadArquivoRetornoWebController extends UploadArquivoWebController {

    @RequestMapping(params = { "acao=carregar" })
    public String carregarRetorno(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) {
        ...
        return carregar(tipo, true, comentario, selecionaEstOrgUploadMargemRetorno, selecionaEstOrgUploadContracheque, TipoArquivoEnum.ARQUIVO_RETORNO_INTEGRACAO, request, response, session, model);
    }

    @RequestMapping(method = { RequestMethod.POST }, params = { "acao=upload" })
    public String uploadRetorno(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) throws UsuarioControllerException, IOException, ValidaImportacaoControllerException, ConsignatariaControllerException, PeriodoException {
        AcessoSistema responsavel = JspHelper.getAcessoSistema(request);
        String tipo = "retorno";
        boolean selecionaEstOrgUploadMargemRetorno = responsavel.isCseSup() && ParamSist.getBoolParamSist(CodedValues.TPC_PERMITE_SELEC_ENT_UPL_ARQ_MARGEM_RETORNO, responsavel);
        boolean selecionaEstOrgUploadContracheque = false;
        ...
        return carregarRetorno(request, response, session, model);
    }
    ...
    protected List<String> recuperarPath(String tipo, boolean selecionaEstOrgUploadMargemRetorno, boolean selecionaEstOrgUploadContracheque, Model model, String papCodigo, String orgCodigo, String estCodigo, String csaCodigo, String corCodigo, boolean temPermissaoEst, AcessoSistema responsavel) throws ZetraException {
        String path = "retorno" + java.io.File.separatorChar;
