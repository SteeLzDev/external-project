...
/**
 * <p>Title: IndeferirConsignacaoWebController</p>
 * <p>Description: Controlador Web para o caso de uso IndeferirConsignacao.</p>
 ...
 */
@Controller
@RequestMapping(method = { RequestMethod.POST }, value = { "/v3/indeferirConsignacao" })
public class IndeferirConsignacaoWebController extends AbstractEfetivarAcaoConsignacaoWebController {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(IndeferirConsignacaoWebController.class);
    ...
    public String efetivarAcao(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) {
        ...
        final String urlDestino = "../v3/indeferirConsignacao?acao=indeferirConsignacao";
        final String funCodigo = CodedValues.FUN_INDF_CONSIGNACAO;
    ...
    @RequestMapping(params = { "acao=indeferirConsignacao" })
    public String indeferirConsignacao(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) throws InstantiationException, IllegalAccessException, ParametroControllerException, ServidorControllerException, ServletException, IOException, ParseException {
        final AcessoSistema responsavel = JspHelper.getAcessoSistema(request);
        ...
        boolean podeDeferir = false;
        if (responsavel.temPermissao(CodedValues.FUN_DEF_CONSIGNACAO)) {
            podeDeferir = true;
        }
        ...
        if (TextHelper.isNull(request.getParameter("ADE_CODIGO"))) {
            ...
            adeCodigosIndef = new String[1];
            adeCodigosIndef[0] = request.getParameter("ADE_CODIGO");
        }
        if (((adeCodigosIndef == null) || (adeCodigosIndef.length == 0)) && (!podeDeferir || (adeCodigosDef == null) || (adeCodigosDef.length == 0))) {
            session.setAttribute(CodedValues.MSG_ERRO, ApplicationResourcesHelper.getMessage("mensagem.erro.interno.contate.administrador", responsavel));
            return viewRedirect("jsp/visualizarPaginaErro/visualizarMensagem", request, session, model, responsavel);
        }
        session.setAttribute(CodedValues.MSG_INFO, ApplicationResourcesHelper.getMessage("mensagem.indeferir.consignacao.concluido.sucesso", responsavel));
    ...
    protected void configurarPagina(HttpServletRequest request, HttpSession session, Model model, AcessoSistema responsavel) {
        if (responsavel.temPermissao(CodedValues.FUN_DEF_CONSIGNACAO)) {
            ...
            model.addAttribute("tituloPagina", ApplicationResourcesHelper.getMessage("rotulo.indeferir.consignacao.titulo", responsavel));
        }

        model.addAttribute("acaoFormulario", "../v3/indeferirConsignacao");
    ...
    protected List<AcaoConsignacao> definirAcoesListaConsignacao(HttpServletRequest request, AcessoSistema responsavel) {
        ...
        if (responsavel.temPermissao(CodedValues.FUN_DEF_CONSIGNACAO)) {
            ...
            String link = "../v3/indeferirConsignacao?acao=" + (deferimentoPelaCsa ? "indeferirConsignacao" : "efetivarAcao");
            ...
            // Adiciona opção para indeferir consignação
            final String link = "../v3/indeferirConsignacao?acao=" + (deferimentoPelaCsa ? "indeferirConsignacao" : "efetivarAcao");
            final String descricao = ApplicationResourcesHelper.getMessage("rotulo.acoes.indeferir.abreviado", responsavel);
            ...
            final String msgConfirmacao = ApplicationResourcesHelper.getMessage("mensagem.confirmacao.multiplo.indeferir", responsavel);
        ...
        if (!deferimentoPelaCsa) {
            ...
            final String link = "../v3/indeferirConsignacao?acao=detalharConsignacao";
        ...
        if (isCse) {
            ...
            final String link = "../v3/indeferirConsignacao?acao=listarConsignacaoSer";
    ...
    protected TransferObject recuperarCriteriosPesquisaPadrao(HttpServletRequest request, AcessoSistema responsavel) {
        ...
        criterio.setAttribute("TIPO_OPERACAO", "indeferir");
