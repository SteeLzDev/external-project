...
/**
 * <p>Title: ReinicializarSenhaUsuarioCorrespondenteWebController</p>
 * <p>Description: Controlador Web base para o caso de uso Reinicializar Senha Usuário Correspondente.</p>
 ...
 */
@Controller
@RequestMapping(method = { RequestMethod.POST }, value = { "/v3/reinicializarSenhaUsuarioCor" })
public class ReinicializarSenhaUsuarioCorrespondenteWebController extends ConsultarUsuarioCorrespondenteWebController {
    ...
    protected void configurarPagina(HttpServletRequest request, HttpSession session, Model model, AcessoSistema responsavel) throws ViewHelperException {
        ...
        String tituloPagina = ApplicationResourcesHelper.getMessage("rotulo.efetiva.acao.usuario.reinicializar.senha", responsavel);
        String msgConfirmacao = ApplicationResourcesHelper.getMessage("mensagem.confirmacao.reinicializar.senha.usuario", responsavel, usuLogin);
    ...
    public String efetivarAcao(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) {
        ...
        String urlDestino = getLinkReinicializarSenhaUsuario() + "?acao=reinicializar";
        String funCodigo = CodedValues.FUN_REINICIALIZAR_SENHA_COR;
        String usuCodigo = JspHelper.verificaVarQryStr(request, "USU_CODIGO");
