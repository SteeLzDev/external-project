...
/**
 * <p>Title: UploadArquivoMargemComplementarWebController</p>
 * <p>Description: Controlador Web para o caso de uso Upload Arquivo Margem Complementar.</p>
 ...
 */
@Controller
@RequestMapping(method = { RequestMethod.POST }, value = { "/v3/uploadArquivoMargemcomplementar" })
public class UploadArquivoMargemComplementarWebController extends UploadArquivoWebController {

    @RequestMapping(params = { "acao=carregar" })
    public String carregarMargemComplementar(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) {
        AcessoSistema responsavel = JspHelper.getAcessoSistema(request);
        String tipo = "margemcomplementar";
        ...
        return carregar(tipo, true, comentario, selecionaEstOrgUploadMargemRetorno, selecionaEstOrgUploadContracheque, TipoArquivoEnum.ARQUIVO_CADASTRO_MARGEM_COMPLEMENTAR, request, response, session, model);
    }


    @RequestMapping(method = { RequestMethod.POST }, params = { "acao=upload" })
    public String uploadMargemComplementar(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) throws UsuarioControllerException, IOException, ValidaImportacaoControllerException, ConsignatariaControllerException, PeriodoException {
        AcessoSistema responsavel = JspHelper.getAcessoSistema(request);
        String tipo = "margemcomplementar";
        boolean selecionaEstOrgUploadMargemRetorno = responsavel.isCseSup() && ParamSist.getBoolParamSist(CodedValues.TPC_PERMITE_SELEC_ENT_UPL_ARQ_MARGEM_RETORNO, responsavel);
        boolean selecionaEstOrgUploadContracheque = false;
        ...
        return carregarMargemComplementar(request, response, session, model);
    }
    ...
    protected List<String> recuperarPath(String tipo, boolean selecionaEstOrgUploadMargemRetorno, boolean selecionaEstOrgUploadContracheque, Model model, String papCodigo, String orgCodigo, String estCodigo, String csaCodigo, String corCodigo, boolean temPermissaoEst, AcessoSistema responsavel) throws ZetraException {
        String path = "margemcomplementar" + java.io.File.separatorChar;
