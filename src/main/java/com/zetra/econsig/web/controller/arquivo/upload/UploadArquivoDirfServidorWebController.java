...
/**
 * <p>Title: UploadArquivoDirfServidorWebController</p>
 * <p>Description: Controlador Web para o caso de uso Upload Arquivo de DIRF de Servidor.</p>
 ...
 */
@Controller
@RequestMapping(method = { RequestMethod.POST }, value = { "/v3/uploadArquivoDirf" })
public class UploadArquivoDirfServidorWebController extends UploadArquivoWebController {

    private static final String TIPO_ARQUIVO = "dirf";

    @RequestMapping(params = { "acao=carregar" })
    public String carregarDirfServidor(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) {
        return carregar(TIPO_ARQUIVO, true, false, false, false, TipoArquivoEnum.ARQUIVO_DIRF_SERVIDOR, request, response, session, model);
    }

    @RequestMapping(method = { RequestMethod.POST }, params = { "acao=upload" })
    public String uploadDirfServidor(HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) throws UsuarioControllerException, IOException, ValidaImportacaoControllerException, ConsignatariaControllerException, PeriodoException {
        AcessoSistema responsavel = JspHelper.getAcessoSistema(request);

        try {
            upload(TIPO_ARQUIVO, false, false, false, request, response, session, model);
        } catch (ZetraException ex) {
            session.setAttribute(CodedValues.MSG_ERRO, ex.getMessage());
            return viewRedirect("jsp/visualizarPaginaErro/visualizarMensagem", request, session, model, responsavel);
        }

        return carregarDirfServidor(request, response, session, model);
    }
    ...
    protected List<String> recuperarPath(String tipo, boolean selecionaEstOrgUploadMargemRetorno, boolean selecionaEstOrgUploadContracheque, Model model, String papCodigo, String orgCodigo, String estCodigo, String csaCodigo, String corCodigo, boolean temPermissaoEst, AcessoSistema responsavel) throws ZetraException {
        String path = TIPO_ARQUIVO + java.io.File.separatorChar;
