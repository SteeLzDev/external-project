...
/**
 * <p>Title: MySqlArquivoRescisaoDAO</p>
 ...
 * $Date$
 */
public class MySqlArquivoRescisaoDAO implements ArquivoRescisaoDAO {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(MySqlArquivoRescisaoDAO.class);
    ...
    public void gerarArquivoRescisao(String nomeArqSaida, String nomeArqConfEntrada, String nomeArqConfTradutor, String nomeArqConfSaida, AcessoSistema responsavel) throws DAOException {
        try {
            ...
            if (gravarDadosRescisao() != 0) {
                ...
                Date periodo = PeriodoHelper.getInstance().getPeriodoAtual(null, responsavel);
                if (ParamSist.getBoolParamSist(CodedValues.TPC_CONCLUIR_CONTRATOS_APOS_GERAR_ARQ_RESCISAO, responsavel)) {                    
                    concluirContratosAposGerarRescisao(periodo);
                }
                
                registrarOcorrenciaParticipacaoConsignacaoRescisaoRse(periodo);
            }else {
                LOG.warn(ApplicationResourcesHelper.getMessage("rotulo.agendamento.movimento.rescisao", responsavel));
            }
        } catch (DAOException | PeriodoException ex) {
    ...
    private int gravarDadosRescisao() throws DAOException {
        ...
        try {
            ...
            query = "insert into tb_tmp_exportacao_arquivo_rescisao (ser_codigo, ser_nome, ser_primeiro_nome , ser_nome_meio , ser_ultimo_nome , ser_nome_pai, ser_nome_mae, ser_cpf, ser_nacionalidade, rse_matricula, rse_matricula_inst, rse_tipo, rse_associado, pos_codigo, " +
                    ...
                    "inner join tb_natureza_servico nse on (svc.nse_codigo = nse.nse_codigo) " +
                    ...
                    " or (svc.nse_codigo = '" + CodedValues.NSE_CARTAO + "' and ade.sad_codigo in ('" + TextHelper.join(CodedValues.SAD_CODIGOS_ATIVOS, "','") + "')) " +
                    " or (nse.nse_retem_verba = 'S' " + " and ade.sad_codigo in ('" + TextHelper.join(CodedValues.SAD_CODIGOS_ATIVOS, "','") + "'))) "
                    ;
    ...
    public void concluirContratosAposGerarRescisao(Date periodo) throws DAOException {
        final NamedParameterJdbcTemplate jdbc = DBHelper.getNamedParameterJdbcTemplate();
        final MapSqlParameterSource queryParams = new MapSqlParameterSource();
        try {
            int linhasAfetadas = 0;
            ...
            query = "update tb_aut_desconto aut " +
                    "inner join tb_tmp_exportacao_arquivo_rescisao tmp on tmp.ade_codigo = aut.ade_codigo " +
                    "set aut.sad_codigo = " + CodedValues.SAD_CONCLUIDO;

            LOG.trace(query);
            linhasAfetadas = jdbc.update(query, queryParams);
            LOG.trace("Linhas afetadas: " + linhasAfetadas);

            query =  "INSERT INTO tb_ocorrencia_autorizacao (oca_codigo, ade_codigo, oca_data, oca_periodo, toc_codigo, usu_codigo, oca_obs) "
                    + "SELECT CONCAT('Z',"
                    + " DATE_FORMAT(NOW(), '%y%m%d%H%i%S'),"
                    + " SUBSTRING(LPAD(tmp.ade_numero, 12, '0'), 1, 12),"
                    + " SUBSTRING(LPAD(@rownum := COALESCE(@rownum, 0) + 1, 7, '0'), 1, 7)), "
                    + "tmp.ade_codigo, NOW(), '" + periodo + "'," + CodedValues.TOC_CONCLUSAO_CONTRATO + ", " + AcessoSistema.getAcessoUsuarioSistema() + ", '" + ApplicationResourcesHelper.getMessage("mensagem.ocorrencia.conclusao.apos.gerar.rescisao" ,(AcessoSistema) null) + "'  FROM tb_tmp_exportacao_arquivo_rescisao tmp ";
    ...
    private void registrarOcorrenciaParticipacaoConsignacaoRescisaoRse(Date periodo) throws DAOException {
        final NamedParameterJdbcTemplate jdbc = DBHelper.getNamedParameterJdbcTemplate();
        final MapSqlParameterSource queryParams = new MapSqlParameterSource();
        try {
            int linhasAfetadas = 0;
            ...
            query =  "INSERT INTO tb_ocorrencia_autorizacao (oca_codigo, ade_codigo, oca_data, oca_periodo, toc_codigo, usu_codigo, oca_obs) "
                    ...
                    + " SUBSTRING(LPAD(tmp.ade_numero, 12, '0'), 1, 12),"
                    + " SUBSTRING(LPAD(@rownum := COALESCE(@rownum, 0) + 1, 7, '0'), 1, 7)), "
                    + "tmp.ade_codigo, NOW(), :periodo," + CodedValues.TOC_REGISTRO_PARTICIPACAO_CONSIGNACAO_RESCISAO_RSE + ", " + AcessoSistema.getAcessoUsuarioSistema() + ", '" + ApplicationResourcesHelper.getMessage("mensagem.ocorrencia.registra.participacao.consignacao.rescisao.registro.servidor" ,(AcessoSistema) null) + "'  FROM tb_tmp_exportacao_arquivo_rescisao tmp ";
