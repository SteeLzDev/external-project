...
/**
 * <p>Title: MySqlControleSaldoDvExpMovimentoDAO</p>
 ...
 * $Date$
 */
public class MySqlControleSaldoDvExpMovimentoDAO extends GenericControleSaldoDvExpMovimentoDAO {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(MySqlControleSaldoDvExpMovimentoDAO.class);

    private static final String TMP_TABLE_CORRECAO_SALDO_DEVEDOR = "tmp_valor_correcao_saldo_devedor";
    ...
    protected void criarTabelaTemporariaCorrecaoSdv() throws DataAccessException {
        ...
        query.append("DROP TEMPORARY TABLE IF EXISTS ").append(TMP_TABLE_CORRECAO_SALDO_DEVEDOR);
        executeUpdate(query.toString(), queryParams);

        // Cria a tabela temporária
        query.setLength(0);
        query.append("CREATE TEMPORARY TABLE ").append(TMP_TABLE_CORRECAO_SALDO_DEVEDOR);
        query.append(" (ADE_CODIGO CHAR(32), ADE_NUMERO INT(10) UNSIGNED, SVC_CODIGO CHAR(32), VLR DECIMAL(13,2), PEX_PERIODO_POS DATE)");
        executeUpdate(query.toString(), queryParams);
    }
    ...
    protected void calcularCorrecaoTotalSaldoDevedor(List<String> servicos) throws DataAccessException {
        ...
        query.append(Columns.CNV_SVC_CODIGO).append(" AS SVC_CODIGO, TRUNCATE(");
        ...
        query.append(" AND ((").append(Columns.CCR_ANO).append(" * 12) + ").append(Columns.CCR_MES).append(") <= ");
        query.append(" ((").append(" YEAR(").append(Columns.PEX_PERIODO).append(") * 12) + ");
        query.append(" MONTH(").append(Columns.PEX_PERIODO).append("))");
        query.append(" ) ");
    ...
    protected void calcularCorrecaoSaldoParcelas(List<String> servicos) throws DataAccessException {
        ...
        query.append(" LEAST(");
        query.append(" SUM(TRUNCATE((").append(Columns.PRD_VLR_PREVISTO).append(" - ");
        query.append("IFNULL(");
        ...
        query.append(",");
        query.append(" TRUNCATE(").append(Columns.ADE_VLR_SDO_RET).append(" * ");
        query.append(Columns.CCR_VLR).append(", 2)");
        query.append(") AS VLR, PEX_PERIODO_POS ");
        query.append(" FROM ").append(Columns.TB_AUTORIZACAO_DESCONTO).append(" FORCE INDEX (r_50_fk)");
    ...
    protected void corrigirSdvProprioServico() throws DataAccessException {
        ...
        final StringBuilder query = new StringBuilder();
        query.append("UPDATE ").append(Columns.TB_AUTORIZACAO_DESCONTO).append(", ");
        query.append(Columns.TB_PARAM_SVC_CONSIGNANTE).append(", ");
        query.append(TMP_TABLE_CORRECAO_SALDO_DEVEDOR);

        // ADE_VLR_SDO_MOV = ADE_VLR_SDO_RET + VALOR_CORREÇÃO
        query.append(" SET ").append(Columns.ADE_VLR_SDO_MOV).append(" = ");
        query.append(Columns.ADE_VLR_SDO_RET).append(" + VLR ");

        query.append(" WHERE ").append(Columns.ADE_CODIGO).append(" = ");
        query.append(TMP_TABLE_CORRECAO_SALDO_DEVEDOR).append(".ADE_CODIGO");
        query.append(" AND ").append(Columns.PSE_SVC_CODIGO).append(" = ");
    ...
    protected void corrigirSdvEmOutroServico() throws DataAccessException {
        final MapSqlParameterSource queryParams = new MapSqlParameterSource();
        final StringBuilder query = new StringBuilder();
        query.append("UPDATE ").append(Columns.TB_AUTORIZACAO_DESCONTO).append(" adeDestino, ");
        query.append(Columns.TB_AUTORIZACAO_DESCONTO).append(" adeOrigem, ");
        ...
        query.append(" adeDestino.ADE_VLR_SDO_MOV = adeDestino.ADE_VLR_SDO_RET + VLR, ");
        // CONTRATO PRINCIPAL: ADE_VLR_SDO_MOV = ADE_VLR_SDO_RET
        query.append(" adeOrigem.ADE_VLR_SDO_MOV  = adeOrigem.ADE_VLR_SDO_RET ");
        query.append(" WHERE adeDestino.ADE_CODIGO = ").append(Columns.RAD_ADE_CODIGO_DESTINO);
        query.append(" AND adeOrigem.ADE_CODIGO = ").append(Columns.RAD_ADE_CODIGO_ORIGEM);
        query.append(" AND ").append(Columns.RAD_ADE_CODIGO_ORIGEM).append(" = ");
        query.append(TMP_TABLE_CORRECAO_SALDO_DEVEDOR).append(".ADE_CODIGO");
        ...
        query.append(CodedValues.TPS_POSSUI_CORRECAO_SALDO_DEVEDOR).append("'");
        query.append(" AND ").append(Columns.PSE_VLR).append(" = '");
        query.append(CodedValues.CORRECAO_SALDO_DEVEDOR_EM_OUTRO_SERVICO).append("'");
        executeUpdate(query.toString(), queryParams);


        // Cria duas tabelas temporárias, pois o MySQL não permite referenciar a mesma
        // tabela duas vezes na mesma consulta
        query.setLength(0);
        query.append("DROP TEMPORARY TABLE IF EXISTS tmp_valor_correcao_saldo_devedor_origem");
        executeUpdate(query.toString(), queryParams);

        query.setLength(0);
        query.append("CREATE TEMPORARY TABLE tmp_valor_correcao_saldo_devedor_origem");
        query.append(" (ADE_CODIGO CHAR(32), ADE_NUMERO INT(10) UNSIGNED, SVC_CODIGO CHAR(32), VLR DECIMAL(13,2), PEX_PERIODO_POS DATE)");
        executeUpdate(query.toString(), queryParams);

        query.setLength(0);
        query.append("INSERT INTO tmp_valor_correcao_saldo_devedor_origem (ADE_CODIGO, ADE_NUMERO, SVC_CODIGO, VLR, PEX_PERIODO_POS)");
        query.append("SELECT tmpOrigem.ADE_CODIGO, tmpOrigem.ADE_NUMERO, tmpOrigem.SVC_CODIGO, tmpOrigem.VLR, tmpOrigem.PEX_PERIODO_POS FROM ");
        query.append(TMP_TABLE_CORRECAO_SALDO_DEVEDOR).append(" tmpOrigem, ");
        query.append(Columns.TB_PARAM_SVC_CONSIGNANTE).append(" paramServicoOrigem ");
        query.append(" WHERE paramServicoOrigem.SVC_CODIGO = tmpOrigem.SVC_CODIGO");
        // O serviço principal tem correção em outro servico
        query.append(" AND paramServicoOrigem.TPS_CODIGO = '");
        query.append(CodedValues.TPS_POSSUI_CORRECAO_SALDO_DEVEDOR).append("'");
        query.append(" AND paramServicoOrigem.PSE_VLR = '");
        query.append(CodedValues.CORRECAO_SALDO_DEVEDOR_EM_OUTRO_SERVICO).append("'");
        executeUpdate(query.toString(), queryParams);

        query.setLength(0);
        query.append("DROP TEMPORARY TABLE IF EXISTS tmp_valor_correcao_saldo_devedor_destino");
        executeUpdate(query.toString(), queryParams);

        query.setLength(0);
        query.append("CREATE TEMPORARY TABLE tmp_valor_correcao_saldo_devedor_destino");
        query.append(" (ADE_CODIGO CHAR(32), ADE_NUMERO INT(10) UNSIGNED, SVC_CODIGO CHAR(32), VLR DECIMAL(13,2), PEX_PERIODO_POS DATE)");
        executeUpdate(query.toString(), queryParams);

        query.setLength(0);
        query.append("INSERT INTO tmp_valor_correcao_saldo_devedor_destino (ADE_CODIGO, ADE_NUMERO, SVC_CODIGO, VLR, PEX_PERIODO_POS)");
        query.append("SELECT tmpDestino.ADE_CODIGO, tmpDestino.ADE_NUMERO, tmpDestino.SVC_CODIGO, tmpDestino.VLR, tmpDestino.PEX_PERIODO_POS FROM ");
        query.append(TMP_TABLE_CORRECAO_SALDO_DEVEDOR).append(" tmpDestino, ");
        query.append(Columns.TB_PARAM_SVC_CONSIGNANTE).append(" paramServicoDestino ");
        query.append(" WHERE paramServicoDestino.SVC_CODIGO = tmpDestino.SVC_CODIGO");
        // O serviço de correção possui tb correção
        query.append(" AND paramServicoDestino.TPS_CODIGO = '");
        query.append(CodedValues.TPS_POSSUI_CORRECAO_SALDO_DEVEDOR).append("'");
        query.append(" AND paramServicoDestino.PSE_VLR = '");
        query.append(CodedValues.CORRECAO_SALDO_DEVEDOR_PROPRIO_SERVICO).append("'");
        ...
        query.append("UPDATE ").append(Columns.TB_AUTORIZACAO_DESCONTO).append(", ");
        query.append(Columns.TB_RELACIONAMENTO_AUTORIZACAO).append(", ");
        query.append("tmp_valor_correcao_saldo_devedor_destino tmpDestino, ");
        query.append("tmp_valor_correcao_saldo_devedor_origem tmpOrigem ");
        query.append(" SET ");
        ...
        query.append(" + tmpDestino.VLR + tmpOrigem.VLR ");
        query.append(" WHERE ").append(Columns.ADE_CODIGO).append(" = ");
        query.append(Columns.RAD_ADE_CODIGO_DESTINO);
        query.append(" AND ").append(Columns.ADE_CODIGO);
        query.append(" = tmpDestino.ADE_CODIGO");
        query.append(" AND ").append(Columns.RAD_ADE_CODIGO_ORIGEM);
        query.append(" = tmpOrigem.ADE_CODIGO");
        query.append(" AND ").append(Columns.RAD_TNT_CODIGO).append(" = '");
        query.append(CodedValues.TNT_CORRECAO_SALDO).append("'");
        executeUpdate(query.toString(), queryParams);
    }
    ...
    protected void criarOcorrenciasCorrecao() throws DataAccessException {
        ...
        query.append(CodedValues.TOC_CORRECAO_SALDO_DEVEDOR).append("' AS TOC_CODIGO, '");
        query.append(CodedValues.USU_CODIGO_SISTEMA).append("' AS USU_CODIGO, NOW() AS OCA_DATA, PEX_PERIODO_POS AS OCA_PERIODO, ");
        query.append("CONCAT('").append(ApplicationResourcesHelper.getMessage("mensagem.ocorrencia.autorizacao.correcao.saldo.devedor", (AcessoSistema) null)).append(": ").append(ApplicationResourcesHelper.getMessage("rotulo.moeda", (AcessoSistema) null)).append(" ', REPLACE(VLR, '.', ',')) AS OCA_OBS");
    ...
    protected void removerOcorrenciasCorrecaoDoPeriodo() throws DataAccessException {
        ...
        query.append(" USING ").append(Columns.TB_OCORRENCIA_AUTORIZACAO).append(", ");
        query.append(Columns.TB_AUTORIZACAO_DESCONTO).append(", ");
        query.append(Columns.TB_VERBA_CONVENIO).append(", ");
        query.append(Columns.TB_CONVENIO).append(", ");
        query.append(Columns.TB_PERIODO_EXPORTACAO);
        query.append(" WHERE ").append(Columns.OCA_ADE_CODIGO).append(" = ");
        query.append(Columns.ADE_CODIGO);
        query.append(" AND ").append(Columns.ADE_VCO_CODIGO).append(" = ");
        query.append(Columns.VCO_CODIGO);
        query.append(" AND ").append(Columns.VCO_CNV_CODIGO).append(" = ");
        query.append(Columns.CNV_CODIGO);
        query.append(" AND ").append(Columns.CNV_ORG_CODIGO).append(" = ");
        query.append(Columns.PEX_ORG_CODIGO);
        // TOC_CORRECAO_SALDO_DEVEDOR
        query.append(" AND ").append(Columns.OCA_TOC_CODIGO).append(" = '");
        query.append(CodedValues.TOC_CORRECAO_SALDO_DEVEDOR).append("'");
        // Ocorrências após o período de exportação
        query.append(" AND ").append(Columns.OCA_DATA).append(" > ");
        query.append(Columns.PEX_DATA_FIM);
        executeUpdate(query.toString(), queryParams);
    }
    ...
    protected void ajustarAdeValorSemControleTeto(List<String> servicosControleSaldo) throws DataAccessException {
        ...
        query.append(Columns.TB_VERBA_CONVENIO).append(", ");
        query.append(Columns.TB_CONVENIO).append(", ");
        query.append(Columns.TB_PARAM_SVC_CONSIGNANTE);
        query.append(" SET ").append(Columns.ADE_VLR).append(" = IFNULL(");
        ...
        query.append(" = ").append(Columns.CNV_CODIGO);
        query.append(" AND ").append(Columns.CNV_SVC_CODIGO);
        query.append(" = ").append(Columns.PSE_SVC_CODIGO);
        // Serviços que possuem controle de saldo devedor
        query.append(" AND ").append(Columns.CNV_SVC_CODIGO);
        query.append(" IN ('").append(TextHelper.join(servicosControleSaldo, "','")).append("')");
        // Contratos que não foram concluidos
        ...
        query.append(" AND ").append(Columns.PSE_TPS_CODIGO).append(" = '");
        query.append(CodedValues.TPS_CONTROLA_VLR_MAX_DESCONTO).append("'");
        query.append(" AND (").append(Columns.PSE_VLR).append(" IS NULL");
        query.append(" OR ").append(Columns.PSE_VLR);
        query.append(" = '").append(CodedValues.NAO_CONTROLA_VLR_MAX_DESCONTO).append("')");
        executeUpdate(query.toString(), queryParams);
    }
    ...
    protected void ajustarAdeValorComTetoDeParcela(List<String> servicosControleSaldo) throws DataAccessException {
        ...
        query.append(" AND ").append(Columns.PSE_TPS_CODIGO).append(" = '");
        query.append(CodedValues.TPS_CONTROLA_VLR_MAX_DESCONTO).append("'");
        query.append(" AND ").append(Columns.PSE_VLR);
        query.append(" = '").append(CodedValues.CONTROLA_VLR_MAX_DESCONTO_PELA_PARCELA).append("'");
        // Apenas se o ADE_VLR for maior que o saldo
        query.append(" AND ").append(Columns.ADE_VLR);
        query.append(" > ").append(Columns.ADE_VLR_SDO_MOV);
        executeUpdate(query.toString(), queryParams);
    }
    ...
    protected void ajustarAdeValorComTetoPeloCargo(List<String> servicosControleSaldo) throws DataAccessException {
        ...
        query.append(CodedValues.TPS_CONTROLA_VLR_MAX_DESCONTO).append("'");
        query.append(" AND ").append(Columns.PSE_VLR);
        query.append(" = '").append(CodedValues.CONTROLA_VLR_MAX_DESCONTO_PELO_CARGO).append("'");
        executeUpdate(query.toString(), queryParams);
        ...
        query.append(Columns.TB_CONVENIO).append(", ");
        query.append(Columns.TB_PARAM_SVC_CONSIGNANTE).append(", ");
        query.append(Columns.TB_REGISTRO_SERVIDOR).append(", ");
        query.append(Columns.TB_CARGO_REGISTRO_SERVIDOR);
        // ADE_VLR = LEAST(ADE_VLR_SDO_MOV, VLR_MAX_DESC)
        query.append(" SET ").append(Columns.ADE_VLR).append(" = IFNULL(LEAST(");
        query.append(Columns.ADE_VLR_SDO_MOV).append(",");
        query.append(Columns.CRS_VLR_DESC_MAX).append("), 0.00)");
        query.append(" WHERE ").append(Columns.ADE_VCO_CODIGO);
        query.append(" = ").append(Columns.VCO_CODIGO);
        query.append(" AND ").append(Columns.VCO_CNV_CODIGO);
        query.append(" = ").append(Columns.CNV_CODIGO);
        query.append(" AND ").append(Columns.CNV_SVC_CODIGO);
        query.append(" = ").append(Columns.PSE_SVC_CODIGO);
        query.append(" AND ").append(Columns.ADE_RSE_CODIGO);
        query.append(" = ").append(Columns.RSE_CODIGO);
        query.append(" AND ").append(Columns.RSE_CRS_CODIGO);
        query.append(" = ").append(Columns.CRS_CODIGO);
        // Serviços que possuem controle de saldo devedor
        query.append(" AND ").append(Columns.CNV_SVC_CODIGO);
        query.append(" IN ('").append(TextHelper.join(servicosControleSaldo, "','")).append("')");
        // Contratos que não foram concluidos
    ...
    public void ajustarValorParcelasAbertas() throws DAOException {
        try {
            ...
            if (servicosControleSaldo != null && servicosControleSaldo.size() > 0) {
                ...
                query.append(" INNER JOIN ").append(Columns.TB_AUTORIZACAO_DESCONTO);
                query.append(" ON (").append(Columns.PDP_ADE_CODIGO).append(" = ").append(Columns.ADE_CODIGO).append(")");
