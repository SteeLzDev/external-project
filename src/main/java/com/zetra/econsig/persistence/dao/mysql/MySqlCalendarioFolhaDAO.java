...
/**
 * <p>Title: MySqlCalendarioFolhaDAO</p>
 * <p>Description: Implementação do DAO de CalendarioFolha para MySQL.</p>
 ...
 * $Date$
 */
public class MySqlCalendarioFolhaDAO implements CalendarioFolhaDAO {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(MySqlCalendarioFolhaDAO.class);
    ...
    public void consolidarCalendarioFolha(List<String> orgCodigos, List<String> estCodigos, AcessoSistema responsavel) throws DAOException {
        if (!PeriodoHelper.folhaMensal(responsavel)) {
            ...
            try {
                ...
                query.append(" ((year(coalesce(coalesce(cfo.cfo_periodo, cfe.cfe_periodo), cfc.cfc_periodo)) * ").append(qtdPeriodos).append(") +");
                query.append("    coalesce(coalesce(cfo.cfo_num_periodo, cfe.cfe_num_periodo), cfc.cfc_num_periodo)) ");
    ...
    public void criarTabelaCalendarioQuinzenal(AcessoSistema responsavel) throws DAOException {
        ...
        try {
            final StringBuilder query = new StringBuilder();
            query.append("drop table if exists tb_tmp_calendario_quinzenal");
            LOG.trace(query.toString());
            jdbc.update(query.toString(), queryParams);
            ...
            query.append("data_ini datetime not null, ");
            query.append("data_fim datetime not null, ");
            query.append("sequencia int not null default 0, ");
            query.append("primary key (org_codigo, periodo), ");
            query.append("key ix01 (org_codigo, sequencia)");
            query.append(")");
            LOG.trace(query.toString());
            jdbc.update(query.toString(), queryParams);
        } catch (final DataAccessException ex) {
