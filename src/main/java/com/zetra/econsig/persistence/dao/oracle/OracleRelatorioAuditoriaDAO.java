 ...
 * $Date$
 */
public class OracleRelatorioAuditoriaDAO extends GenericRelatorioAuditoriaDAO {

    /** Log object for this class. */
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(OracleRelatorioAuditoriaDAO.class);
    ...
    public String select(Map<String, Object> parametros, MapSqlParameterSource queryParams) throws DAOException {
        ...
        boolean auditoria = !TextHelper.isNull(getParametro("MODULO_AUDITORIA", parametros)) && getParametro("MODULO_AUDITORIA", parametros).toString().equals("true");
        ...
        sql.setLength(0);
        sql.append("SELECT ");
        sql.append("CASE WHEN ").append(Columns.USU_STU_CODIGO).append(" = '3' then COALESCE(").append(Columns.USU_TIPO_BLOQ).append(", COALESCE(");
        ...
        sql.append("REPLACE(REPLACE(clob2char(").append(campoObs).append("), '<BR>', '\n'), '<br>', '\n') AS LOG_OBS, "); // Operador replace é CASE SENSITIVE
        ...
        if (TextHelper.isNull(tipoEntidadeUsuario) || !tipoEntidadeUsuario.equalsIgnoreCase(AcessoSistema.ENTIDADE_CSA)) {
            ...
            sql.append("TB_CONSIGNATARIA_COR.CSA_NOME|| ' - ' || ").append(Columns.COR_NOME).append("");
        ...
        sql.append("TO_CHAR(").append(campoData).append(", 'DD-MM-YYYY HH24:MI:SS') AS DATA_LOG ");

        if (nomeTabelaResultado.equalsIgnoreCase("tb_resultado_relatorio")) {
    ...
    protected void populaTabelaResultado(Connection conn, String nomeTabelaResultado, String nomeTabela,
        ...
        for (String grupoFuncao : tabelasResult) {
            ...
            if (camposTabelaAuditoria != null && !camposTabelaAuditoria.isEmpty()) {
                ...
                while (ite.hasNext()) {
                    sql.append(Columns.getColumnName(ite.next()));
                    if (ite.hasNext()) {
                        sql.append(", ");
                    }
                }
            ...
            while (iteCampo.hasNext()) {
                ...
                if (!TextHelper.isNull(campoDescricao)) {
                    ...
                    sql.append("COALESCE('").append(Columns.getColumnLabel(campoDescricao)).append(": '").append(" || TO_CHAR(").append(tabelaJoin).append(".").append(descricao).append(") || '\\n', '')");
            ...
            sql.append("LEFT OUTER JOIN ").append(Columns.TB_CONSIGNATARIA).append(" TB_CONSIGNATARIA_COR ON (TB_CONSIGNATARIA_COR.CSA_CODIGO = cor.CSA_CODIGO) ");
            if (TextHelper.isNull(tipoEntidadeUsuario) || !tipoEntidadeUsuario.equals(AcessoSistema.ENTIDADE_CSA)) {
    ...
    protected void criaTabela(String nomeTabela) throws DataAccessException {
        ...
        query.append("TLO_CODIGO                     varchar2(32)    not null, ");
        query.append("TEN_CODIGO_LOG                 varchar2(32), ");
        query.append("USU_CODIGO_LOG                 varchar2(32), ");
        query.append("FUN_CODIGO_LOG                 varchar2(32), ");
        query.append("LOG_DATA                       date           not null, ");
        query.append("LOG_OBS                        clob           not null, ");
        query.append("LOG_IP                         varchar2(45), ");
        query.append("LOG_CANAL                      char(1), ");
        while (ite.hasNext()) {
            String chave = ite.next();
            query.append(Columns.getColumnName(chave).toUpperCase()).append(" varchar2(32), ");
        }
        ...
        String nomeTabelaUpperCase = nomeTabela.toUpperCase();
        int count = 1;
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (TLO_CODIGO) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (TEN_CODIGO_LOG) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (USU_CODIGO_LOG) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (FUN_CODIGO_LOG) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (LOG_DATA) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
    }
    ...
    protected void criaTabelaGrupoFuncao(String nomeTabela, String grupoFuncao) throws DataAccessException {
        ...
        while (iteCampos.hasNext()) {
            query.setLength(0);
            query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (").append(iteCampos.next()).append(")");
            LOG.trace(query.toString());
            jdbc.update(query.toString(), queryParams);
        }
    ...
    protected void criaTabelaResultado(String nomeTabela) throws DataAccessException {
        ...
        query.setLength(0);
        query.append("CREATE TABLE ").append(nomeTabela).append(" (");
        query.append("CODIGO_ENTIDADE                varchar2(32), ");
        query.append("USU_CODIGO                     varchar2(32), ");
        query.append("TLO_CODIGO                     varchar2(32)   not null, ");
        query.append("FUN_CODIGO                     varchar2(32), ");
        query.append("TEN_CODIGO                     varchar2(32), ");
        query.append("AUDITADO                       char(1)        default 'N' not null, ");
        query.append("LOG_IP                         varchar2(45), ");
        query.append("LOG_DATA                       date           not null, ");
        query.append("LOG_OBS                        clob           not null, ");
        query.append("LOG_CANAL                      char(1) ");
        ...
        int count = 1;
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (CODIGO_ENTIDADE)");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (USU_CODIGO) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (TLO_CODIGO) ");
        ...
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (FUN_CODIGO) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count++).append(" ON ").append(nomeTabela).append(" (TEN_CODIGO) ");
        LOG.trace(query.toString());
        jdbc.update(query.toString(), queryParams);
        query.setLength(0);
        query.append("CREATE INDEX ").append(nomeTabelaUpperCase).append("_IDX_").append(count).append(" ON ").append(nomeTabela).append(" (LOG_DATA) ");
    ...
    protected void excluiTabela(String nomeTabela) throws DataAccessException {
        ...
        StringBuilder query = new StringBuilder();
        query.setLength(0);
        query.append("CALL dropTableIfExists('").append(nomeTabela).append("')");
