...
/**
 * <p>Title: ListaExtratoMargemContratosConcluidosFeriasQuery</p>
 ...
 * QUE NÃO LIBERAM MARGEM NA CONCLUSÃO DOS CONTRATOS EM FÉRIAS (TPC 289 = N)
 ...
 * $Date$
 */
public class ListaExtratoMargemContratosConcluidosFeriasQuery extends HQuery {

    private final String rseCodigo;
    private final Date ultPeriodo;
    private final Date dataFimUltPeriodo;

    public ListaExtratoMargemContratosConcluidosFeriasQuery(String rseCodigo, Date ultPeriodo, Date dataFimUltPeriodo) {
    ...
    public Query<Object[]> preparar(Session session) throws HQueryException {
        ...
        sql.append("   WITH oca.tipoOcorrencia.tocCodigo = '").append(CodedValues.TOC_CONCLUSAO_CONTRATO).append("'");
        ...
        sql.append(" AND (ade.statusAutorizacaoDesconto.sadCodigo = '").append(CodedValues.SAD_CONCLUIDO).append("')");
        ...
        if (ParamSist.paramEquals(CodedValues.TPC_LIB_MARGEM_CONCLUSAO_CONTRATO, CodedValues.TPC_SIM, AcessoSistema.getAcessoUsuarioSistema())) {
            sql.append(" AND EXISTS (SELECT 1 FROM ade.parcelaDescontoSet prd ");
        ...
        // Se zera margem usada (default NAO) ou libera margem na conslusão de férias (default SIM)
        // adiciona na query cláusula para não retornar nada.
        if (ParamSist.paramEquals(CodedValues.TPC_ZERA_MARGEM_USADA, CodedValues.TPC_SIM, AcessoSistema.getAcessoUsuarioSistema()) ||
                (!ParamSist.paramEquals(CodedValues.TPC_LIB_MARGEM_CONCLUSAO_CONTRATO_FERIAS, CodedValues.TPC_NAO, AcessoSistema.getAcessoUsuarioSistema()) && ParamSist.paramEquals(CodedValues.TPC_LIB_MARGEM_CONCLUSAO_CONTRATO, CodedValues.TPC_SIM, AcessoSistema.getAcessoUsuarioSistema()))) {
            sql.append(" AND (1 = 2)");
        }
