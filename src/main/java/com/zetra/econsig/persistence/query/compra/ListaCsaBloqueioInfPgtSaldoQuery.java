...
/**
 * <p>Title: ListaCsaBloqueioInfPgtSaldoQuery</p>
 ...
 * $Date$
 */
public class ListaCsaBloqueioInfPgtSaldoQuery extends HNativeQuery {
    ...
    public Query<Object[]> preparar(Session session) throws HQueryException {
        ...
        // Prazo para pagamento de saldo excedeu
        if (ParamSist.paramEquals(CodedValues.TPC_USA_DIAS_UTEIS_CONTROLE_COMPRA, CodedValues.TPC_SIM, AcessoSistema.getAcessoUsuarioSistema())) {
            ...
            corpoBuilder.append(" AND (SELECT COUNT(1) FROM " + Columns.TB_CALENDARIO + " WHERE " + Columns.CAL_DIA_UTIL + " = 'S' AND " + Columns.CAL_DATA + " BETWEEN to_date(" + Columns.RAD_DATA_REF_PGT_SALDO + ") AND data_corrente()) > ");
        } else {
            corpoBuilder.append(" AND (to_days(data_corrente()) - to_days(" + Columns.RAD_DATA_REF_PGT_SALDO + ")) >= ");
        }
        ...
        // A data de referência está preenchida e a de pagamento de saldo não
        corpoBuilder.append("   AND " + Columns.RAD_DATA_REF_PGT_SALDO + " IS NOT NULL AND " + Columns.RAD_DATA_PGT_SALDO + " IS NULL ");

        // A compra está aguardando saldo devedor
        corpoBuilder.append("   AND " + Columns.RAD_STC_CODIGO + " = '" + StatusCompraEnum.AGUARDANDO_PAG_SALDO.getCodigo() + "' ");
