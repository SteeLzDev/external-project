...
/**
 * <p>Title: RelatorioMovFinQuery</p>
 ...
 * $Date$
 */
public class RelatorioMovFinQuery extends ReportHQuery {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(RelatorioMovFinQuery.class);
    ...
    private void setUsoPeriodo(Session session) throws HQueryException {
        final ObtemTotalParcelasQuery totalParcelas = new ObtemTotalParcelasQuery();
        ...
        final ObtemTotalParcelasPeriodoQuery totalParcelasPer = new ObtemTotalParcelasPeriodoQuery();
    ...
    public Query<Object[]> preparar(Session session) throws HQueryException {
        ...
        fields += "str(ade.adeNumero) as ADE_NUM,"
                ...
                + (exibirPostoServidor() ? "concat(rse.rseTipo, (case when nullif(pos.posDescricao, '') is null then '' else concat(' - ', pos.posDescricao) end))" : "rse.rseTipo") + " AS rse_tipo,"
        ...
        if ((useParcelaPeriodo && useParcela) || ((useParcelaPeriodo || useParcela) && !innerJoin)) {
            if (useParcela) {
                ...
                sql.append(" with prd.prdDataDesconto = :periodo");
            ...
            if (useParcelaPeriodo) {
                ...
                sql.append(" with pdp.prdDataDesconto = :periodo");
        ...
        if (exibirPostoServidor()) {
            sql.append(" left outer join rse.postoRegistroServidor pos");
        }
        ...
        sql.append(" and (ade.adeAnoMesIni <= :periodo");
        ...
        final String ordenacao = " order by 1, 2, ser.serNome";
    ...
    private boolean exibirPostoServidor() {
        try {
            return ShowFieldHelper.showField(FieldKeysConstants.RELATORIO_MOV_FIN_POSTO_SERVIDOR, AcessoSistema.getAcessoUsuarioSistema());
