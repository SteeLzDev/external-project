 ...
 * $Date$
 */
public class RelatorioSinteticoGerencialCsaVolumePortabilidadeTipoQuery extends ReportHNativeQuery {
    ...
    public Query<Object[]> preparar(Session session) throws HQueryException {
        ...
        corpo.append(" select '").append(ApplicationResourcesHelper.getMessage("rotulo.relatorio.sinteticogerencialconsignataria.volume.portabilidade.venda", responsavel)).append("' as tipo, csaVendedora.csa_nome as nome, count(*) as quantidade, COALESCE(round(count(*) / (");
        ...
        corpo.append(" ) * 100, 2),0) as percentual");
        corpo.append(" from tb_aut_desconto ade");
        corpo.append(" inner join tb_verba_convenio vco USING (vco_codigo)");
        corpo.append(" INNER join tb_convenio cnv USING (cnv_codigo)");
        corpo.append(" INNER join tb_relacionamento_autorizacao rad ON (rad.tnt_codigo = :nseCompra AND rad.ade_codigo_destino = ade.ade_codigo)");
        ...
        corpo.append(" where rad.rad_data BETWEEN add_month(:periodoIni, -6) AND :periodoFim");
        corpo.append("   AND cnv.csa_codigo = :csaCodigo ");
        corpo.append(" group by csaVendedora.csa_nome");
        corpo.append(" UNION ALL");
        corpo.append(" select '").append(ApplicationResourcesHelper.getMessage("rotulo.relatorio.sinteticogerencialconsignataria.volume.portabilidade.compra", responsavel)).append("' as tipo, csaCompradora.csa_nome as nome, count(*) as quantidade, COALESCE(round(count(*) / (");
        corpo.append("   select count(*)");
        corpo.append("   from tb_aut_desconto ade");
        corpo.append("   inner join tb_verba_convenio vco USING (vco_codigo)");
        corpo.append("   INNER join tb_convenio cnv USING (cnv_codigo)");
        ...
        corpo.append(" where rad.rad_data BETWEEN add_month(:periodoIni, -6) AND :periodoFim");
        corpo.append("   AND cnv.csa_codigo = :csaCodigo ");
        corpo.append(" group by csaCompradora.csa_nome");
        corpo.append(" order by 1 desc");
