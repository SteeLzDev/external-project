...
update tb_tipo_param_sist_consignante set gps_codigo = '1' where tpc_codigo in ('638','639','640');

update tb_tipo_param_sist_consignante set gps_codigo = '3' where tpc_codigo in ('642');


-- 2) CONCEDER A  PERMISSAO DE VISUALIZAR E ALTERAR PARA USUÁRIOS SUP: 

update tb_tipo_param_sist_consignante set tpc_sup_consulta = 'S', tpc_sup_altera = 'S' where tpc_codigo in ('638','639','640','642');
...
where tpc.tpc_codigo in ('638','639','640');

insert ignore into tb_perfil_param_sist_cse
select tpc.tpc_codigo, per.per_codigo
from tb_tipo_param_sist_consignante tpc
inner join tb_perfil per on (per.per_codigo = 'ZETRA-IMP_AVANCADO')
where tpc.tpc_codigo in ('638','639','640','642');
