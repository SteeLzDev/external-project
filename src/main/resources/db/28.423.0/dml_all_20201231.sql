...
update tb_tipo_param_sist_consignante set  gps_codigo = '1' where tpc_codigo in ('794','797','802','803');
update tb_tipo_param_sist_consignante set  gps_codigo = '2' where tpc_codigo in ('793','798','796','800');
update tb_tipo_param_sist_consignante set  gps_codigo = '3' where tpc_codigo in ('795','799');


-- 2) CONCEDER A  PERMISSAO DE VISUALIZAR E ALTERAR PARA USUÁRIOS SUP: 
update tb_tipo_param_sist_consignante set tpc_sup_consulta = 'S', tpc_sup_altera = 'S' where tpc_codigo in ('793','794','795','796','797','798','799','800','802','803');
...
where tpc.tpc_codigo in ('793','794','795','796','797','798','799','800');
...
where tpc.tpc_codigo in ('793','796','798','800'); 

insert ignore into tb_perfil_param_sist_cse (tpc_codigo, per_codigo)
select tpc.tpc_codigo, per.per_codigo
from tb_tipo_param_sist_consignante tpc
inner join tb_perfil per on (per.per_codigo = 'ZETRA-SEGURANCA_BASICO')
where tpc.tpc_codigo in ('794','797','802','803');
