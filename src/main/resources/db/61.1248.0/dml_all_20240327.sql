...
update tb_tipo_param_sist_consignante set  gps_codigo = '1' where tpc_codigo in ('948');
update tb_tipo_param_sist_consignante set  gps_codigo = '2' where tpc_codigo in ('947');
update tb_tipo_param_sist_consignante set  gps_codigo = '3' where tpc_codigo in ('949','950');


-- 2) CONCEDER A  PERMISSAO DE VISUALIZAR E ALTERAR PARA USUÁRIOS SUP
update tb_tipo_param_sist_consignante set tpc_sup_consulta = 'S', tpc_sup_altera ='S' where tpc_codigo in ('947','948','949','950');
...
where tpc.tpc_codigo in ('947','948','949','950')
and not exists (
