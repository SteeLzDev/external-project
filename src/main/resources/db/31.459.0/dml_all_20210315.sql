...
update tb_tipo_param_sist_consignante set gps_codigo = '1' where tpc_codigo in ('811','812');

update tb_tipo_param_sist_consignante set gps_codigo = '3' where tpc_codigo in ('806','807','808');

-- 2) CONCEDER A PERMISSAO DE VISUALIZAR E ALTERAR PARA USUÁRIOS SUP:

update tb_tipo_param_sist_consignante set tpc_sup_consulta = 'S', tpc_sup_altera = 'S' where tpc_codigo in ('806','807','808','811','812');
...
where tpc.tpc_codigo in ('806','807','808','811','812');
...
where tpc.tpc_codigo in ('811','812');
