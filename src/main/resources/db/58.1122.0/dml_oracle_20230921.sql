...
SELECT 'I' || TO_CHAR(SYSDATE, 'YYMMDDHH24MISS') || SUBSTR(LPAD(svc.svc_codigo, 12, '0'), 1, 12) || SUBSTR(LPAD(svc.svc_identificador, 7, '0'), 1, 7), svc.svc_codigo, '325' AS TPS_CODIGO, cse.cse_codigo, pse.pse_vlr, NULL
FROM tb_servico svc
...
INSERT INTO tb_param_svc_consignante (PSE_CODIGO, SVC_CODIGO, TPS_CODIGO, CSE_CODIGO, PSE_VLR, PSE_VLR_REF)
SELECT 'I' || TO_CHAR(SYSDATE, 'YYMMDDHH24MISS') || SUBSTR(LPAD(svc.svc_codigo, 12, '0'), 1, 12) || SUBSTR(LPAD(svc.svc_identificador, 7, '0'), 1, 7), svc.svc_codigo, '326', cse.cse_codigo, pse.pse_vlr, NULL
FROM tb_servico svc
INNER JOIN tb_param_svc_consignante pse ON (pse.svc_codigo = svc.svc_codigo AND pse.tps_codigo = '210')
INNER JOIN tb_consignante cse ON (cse.cse_codigo = pse.cse_codigo);
