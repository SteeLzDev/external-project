...
update tb_tipo_param_sist_consignante set  gps_codigo = '1' where tpc_codigo in ('932','933','934');
update tb_tipo_param_sist_consignante set  gps_codigo = '2' where tpc_codigo in ('931');
update tb_tipo_param_sist_consignante set  gps_codigo = '3' where tpc_codigo in ('935');



-- 2) CONCEDER A  PERMISSAO DE VISUALIZAR E ALTERAR PARA USUÁRIOS SUP:

update tb_tipo_param_sist_consignante set tpc_sup_consulta = 'S', tpc_sup_altera ='S' where tpc_codigo in ('931','932','933','934','935');
...
where tpc.tpc_codigo in ('931','932','933','934','935')
and not exists (
...
where tpc.tpc_codigo in ('931','935')
and not exists (
  select 1 from tb_perfil_param_sist_cse pcc
  where tpc.tpc_codigo = pcc.tpc_codigo
    and per.per_codigo = pcc.per_codigo
...
where tpc.tpc_codigo in ('932','933','934')
and not exists (
 select 1 from tb_perfil_param_sist_cse pcc
 where tpc.tpc_codigo = pcc.tpc_codigo
 and per.per_codigo = pcc.per_codigo
