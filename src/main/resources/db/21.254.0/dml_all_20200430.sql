...
update tb_tipo_param_sist_consignante set gps_codigo = '1' where tpc_codigo in ('722');

update tb_tipo_param_sist_consignante set gps_codigo = '3' where tpc_codigo in ('723');


-- 2) CONCEDER A  PERMISSAO DE VISUALIZAR E ALTERAR PARA USUÁRIOS SUP: 

update tb_tipo_param_sist_consignante set tpc_sup_consulta = 'S', tpc_sup_altera = 'S' where tpc_codigo in ('722','723');
...
where tpc.tpc_codigo in ('722','723');
