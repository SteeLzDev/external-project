...
%>
          <div class="form-group col-sm-12 col-md-6">
            <span id="periodoIniFim">${descricoes[recurso]}</span>
            <div class="row" role="group" aria-labelledby="periodoIniFim">
              <div class="col-sm-6">
                <div class="row">
                  ...
                  <div class="form-check col-sm-10 col-md-10">
                    <hl:htmlinput name="periodoIni" di="periodoIni" type="text" value="<%=TextHelper.forHtmlAttribute(valueIni )%>" others="<%=TextHelper.forHtmlAttribute(others )%>" classe="form-control w-100" size="10" mask="<%=LocaleHelper.getPeriodoPlaceHolder()%>" placeHolder="<%=LocaleHelper.getPeriodoPlaceHolder()%>" />
                  </div>
                </div>
              </div>
              <div class="col-sm-6">
                <div class="row">
                  ...
                  <div class="form-check col-sm-10 col-md-10">
                    <hl:htmlinput name="periodoFim" di="periodoFim" type="text" value="<%=TextHelper.forHtmlAttribute(valueFim )%>" others="<%=TextHelper.forHtmlAttribute(others )%>" classe="form-control w-100" size="10" mask="<%=LocaleHelper.getPeriodoPlaceHolder()%>" placeHolder="<%=LocaleHelper.getPeriodoPlaceHolder()%>" />
                  </div>
                </div>
              </div>
            </div>
    ...
    <% if (obrDataInclusaoPage.equals("true")) { %>                    
      ...
      function funDataInclusaoPage() {
          ...
          msgCamposObrigatorios = msgCamposObrigatorios + '<hl:message key="mensagem.informe.periodo.filtro.inicio"/>,';
          msgCamposObrigatorios = msgCamposObrigatorios + '<hl:message key="mensagem.informe.periodo.filtro.fim"/>,';
      }
    ...
    <% } %>
        ...
        <script type="text/JavaScript">
         function valida_campo_periodo_ini_fim() {
             with(document.forms[0]) {
               ...
               if (periodoIni != null && periodoFim != null) {
                   var diasDif = document.getElementById('agendadoSim') == null || !document.getElementById('agendadoSim').checked ? 365 : '';
