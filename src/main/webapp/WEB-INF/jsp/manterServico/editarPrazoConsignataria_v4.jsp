...
List<PrazoTransferObject> prazosCsa = (List<PrazoTransferObject>) request.getAttribute("prazosCsa");
%>
<c:set var="title">
...
<form name="form1" action="../v3/editarPrazoConsignataria?<%=SynchronizerToken.generateToken4URL(request)%>" method="post">
      ...
      <div class="col-sm-12 col-md-12 mb-2">
        <div class="float-end">
          ...
          <div class="dropdown-menu dropdown-menu-right" aria-labelledby="acoes" x-placement="bottom-end" style="position: absolute; transform: translate3d(1009px, 50px, 0px); top: 0px; left: 0px; will-change: transform;">
            <a class="dropdown-item" href="#no-back" onClick="postData('../v3/editarPrazoConsignataria?acao=bloquearTudo&SVC_CODIGO=<%=TextHelper.forJavaScriptAttribute(svc_codigo)%>&titulo=<%=TextHelper.forJavaScript(titulo)%>&CSA_CODIGO=<%=TextHelper.forJavaScript(csa_codigo)%>&SVC_DESCRICAO=<%=TextHelper.forJavaScript(svc_descricao)%>&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&_skip_history_=true'); return false;"><hl:message key="rotulo.prazo.bloquear.todos.botao"/></a>
            <a class="dropdown-item" href="#no-back" onClick="postData('../v3/editarPrazoConsignataria?acao=desbloquearTudo&SVC_CODIGO=<%=TextHelper.forJavaScriptAttribute(svc_codigo)%>&titulo=<%=TextHelper.forJavaScript(titulo)%>&CSA_CODIGO=<%=TextHelper.forJavaScript(csa_codigo)%>&SVC_DESCRICAO=<%=TextHelper.forJavaScript(svc_descricao)%>&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&_skip_history_=true'); return false;"><hl:message key="rotulo.prazo.desbloquear.todos.botao"/></a>
          </div>
        </div>
      </div>
      <div class="col-sm-5">
              ...
              <div class="form-group col-sm-12 col-md-12">
                <div class="row mt-2" role="group" aria-labelledby="De">
                  ...
                  <div class="form-check pt-2 col-sm-12 col-md-11">
                    <input type="text" class="form-control w-100" name="PRZ_VLR_INI" id="PRZ_VLR_INI" placeholder="<%=ApplicationResourcesHelper.getMessage("rotulo.prazo.de.placeholder", responsavel) %>" onFocus="SetarEventoMascara(this,'#D3',true);" onBlur="fout(this);ValidaMascara(this);" size="5" maxlength="3">
                  </div>
                  <div class="form-check pt-2 col-sm-12 col-md-1 p-2 ">
                    <div class="float-left align-middle mt-4 form-control-label">
                  ...
                  <div class="form-check pt-2 col-sm-12 col-md-11">
                    <input type="text" class="form-control w-100" id="PRZ_VLR_FIM" NAME="PRZ_VLR_FIM" placeholder="<%=ApplicationResourcesHelper.getMessage("rotulo.prazo.ate.placeholder", responsavel) %>" onFocus="SetarEventoMascara(this,'#D3',true);" onBlur="fout(this);ValidaMascara(this);" size="5" maxlength="3">
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="btn-action">
          ...
          <a class="btn btn-primary ml-1" href="#confirmarSenha" onClick="if (vf_interval()) {f0.acao.value = 'desbloquear'; f0.submit();} else {return false;}"><hl:message key="rotulo.prazo.desbloquear"/></a>
          <a class="btn btn-primary ml-1" href="#confirmarSenha" onClick="if (vf_interval()) {f0.acao.value = 'bloquear'; f0.submit();} else {return false;}"><hl:message key="rotulo.prazo.bloquear"/></a>
        </div>
      </div>
    <% } %>
    <div class="col-sm-7">
             ...
             <%
              if (prazos.size() == 0) {
                ...
                while (it.hasNext()) {
                  ...
                  prz_ativo = (prazosCsa.contains(prazo) ? CodedValues.STS_ATIVO : CodedValues.STS_INATIVO);
              ...
              %>
                <tr>
                  ...
                  <% if (podeEditarPrazo) { %>
                    <td><a href="#no-back" onClick="postData('../v3/editarPrazoConsignataria?acao=salvar&SVC_CODIGO=<%=TextHelper.forJavaScriptAttribute(svc_codigo)%>&CSA_CODIGO=<%=TextHelper.forJavaScriptAttribute(csa_codigo)%>&titulo=<%=TextHelper.forJavaScript(titulo)%>&PRZ_CODIGO=<%=TextHelper.forJavaScript(prz_codigo)%>&PRZ_ATIVO=<%=TextHelper.forJavaScript((prz_ativo.equals(CodedValues.STS_ATIVO) ? CodedValues.STS_INATIVO : CodedValues.STS_ATIVO))%>&SVC_DESCRICAO=<%=TextHelper.forJavaScript(svc_descricao)%>&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&_skip_history_=true')"><%=(prz_ativo.equals(CodedValues.STS_ATIVO) ? ApplicationResourcesHelper.getMessage("rotulo.prazo.bloquear", responsavel) : ApplicationResourcesHelper.getMessage("rotulo.prazo.desbloquear", responsavel))%></a>
                  <% } else { %>
...
function vf_interval () {
  ...
  if (ini == null || ini == "") {
    alert ('<hl:message key="mensagem.prazo.preencher.valor.inicial"/>');
    f0.PRZ_VLR_INI.focus();
    return false;
  }
  
  if (fim == null || fim == "") {
    alert ('<hl:message key="mensagem.prazo.preencher.valor.final"/>');
    f0.PRZ_VLR_FIM.focus();
    return false;
  }
