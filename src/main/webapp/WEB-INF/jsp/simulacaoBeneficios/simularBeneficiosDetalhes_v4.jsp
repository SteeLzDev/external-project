...
<c:set var="bodyContent">
  ...
  <form method="post" action="../v3/simulacaoBeneficios?acao=salvar&&_skip_history_&<%=SynchronizerToken.generateToken4URL(request)%>" name="form1" id="form1">
  ...
  <div class="btn-action col-sm">      
    <a href="#no-back" class="btn btn-outline-danger" onClick="postData('../v3/simulacaoBeneficios?acao=<%=TextHelper.forJavaScript(paraOndeVoltar)%>&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&RSE_CODIGO=<%=TextHelper.forJavaScript(rseCodigo)%>'); return false;"><hl:message key="rotulo.botao.voltar"/></a>
