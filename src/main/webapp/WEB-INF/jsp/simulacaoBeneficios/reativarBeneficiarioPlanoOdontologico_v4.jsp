<%--
* <p>Title: reativarBeneficiarioPlanoOdontologico_v4</p>
* <p>Description: Reativar Beneficiarios Plano Odontologico de beneficio Cancelados v4</p>
* <p>Copyright: Copyright (c) 2018</p>
* <p>Company: Nostrum Consultoria e Projetos</p>
* $Author: marcos.nolasco $
* $Revision: 28822 $
* $Date: 2020-03-23 14:24:44 -0300 (sex, 07 fev 2020) $
--%>
...
<%
  ...
  List<ConsignatariaTransferObject> consignatariasOdontologico = (List) request.getAttribute("consignatariasOdontologico");
  ...
  List<String> benCodigos = (List) request.getAttribute("benCodigos");
%>
<c:set var="title">
  <hl:message key="rotulo.reativar.contrato.beneficio.odonto.titulo" />
</c:set>
...
<c:set var="bodyContent">
      ...
      <div class="card">
        ...
        <div class="card-body table-responsive">
          <form>
            <div class="row">
              <div class="col-sm">
                <label for="selectOperadoraPlanoOdontologico"><hl:message key="rotulo.simulacao.beneficio.operadora"/></label>
                <select class="form-control"
                  id="selectOperadoraPlanoOdontologico">
                  <option value="1"><hl:message key="rotulo.simulacao.beneficio.selecione.operadora"/></option>
                  <%
                      for (ConsignatariaTransferObject consignataria : consignatariasOdontologico) {
                  %>
                  <option
            ...
            <div class="row">
              <div class="col-sm">
                <table class="table">
                  <tbody id="detalhesBeneficiosPlanoOdontologico">
                  </tbody>
                </table>
              </div>
            </div>
        ...
        <div class="card-body divTable p-0">
          <div class="divTableBody text-center">
            ...
            <div class="divTableRow">
              ...
              <div class="divTableCell font-weight-bold" id="totalMensalidadePlanoOdontologico"></div>
              <div class="divTableCell font-weight-bold" id="totalSubsidioPlanoOdontologico"></div>
        ...
        <div class="card-body table-responsive ">
          <table class="table table-striped table-hover">
            ...
            <tbody>
              ...
              <tr class="selecionarLinha" id="linha_plano_odontologico_<%=TextHelper.forHtmlAttribute(beneficiario.getAttribute(Columns.BFC_CODIGO))%>" style="display: none;">
                ...
                <td class="selecionarColuna" id="mensalidade_plano_odontologico_<%=TextHelper.forHtmlAttribute(beneficiario.getAttribute(Columns.BFC_CODIGO))%>"></td>
                <td class="selecionarColuna" id="subsidio_plano_odontologico_<%=TextHelper.forHtmlAttribute(beneficiario.getAttribute(Columns.BFC_CODIGO))%>"></td>
            ...
            <tfoot>
              <tr>
                <td colspan="2" id="descFootPlanoOdontologico"><hl:message key="rotulo.simulacao.beneficio.resumo.listagem.resultado.plano.odontologico"/></td>
              </tr>
            </tfoot>
          </table>
        </div>
  ...
  <div class="btn-action col-sm">      
    <a href="#no-back" class="btn btn-outline-danger" onClick="fluxoVoltar()"><hl:message key="rotulo.botao.voltar"/></a>
...
<c:set var="javascript">
  ...
  function fluxoAddBeneficiario() { 
	  postData('../v3/listarBeneficiarios?acao=listar&RSE_CODIGO=<%=TextHelper.forJavaScriptAttribute(rseCodigo)%>&<%=SynchronizerToken.generateToken4URL(request)%>&_skip_history_')
  }
  ...
  function fluxoVoltar() { 
	    ...
	    %>
	    	postData('../v3/reativarBeneficiarioSimulacaoBeneficios?acao=iniciar');
	    <%
	    } else { 
	    %>
	    	postData('../v3/reativarBeneficiarioSimulacaoBeneficios?acao=planoSaude&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&RSE_CODIGO=<%=TextHelper.forJavaScript(rseCodigo)%>'); return false;	    <%
	    } 
	    %>
  }
		  ...
		  try{
			  if (data != null) {
	    		  ...
	    		  if (dataJ.rseCodigo == '<%=TextHelper.forJavaScript(rseCodigo)%>'){
	    			  ...
	    			  if (dataNse != null) {    			  
	        			  $('#selectOperadoraPlanoOdontologico option[value="'+dataNse.csaCodigo+'"]').attr("selected",true);
  ...
  $('#selectOperadoraPlanoOdontologico').change(function () {
	  	showModal();
  ...
  function carregaBeneficiosAjax(csaCodigo, elementoDefault) {
	  return $.ajax({
	        type: 'POST',
	        url: '../v3/reativarBeneficiarioSimulacaoBeneficios/carregaBeneficiosAjax?<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&_skip_history_',
	        data: {
	            'csaCodigo': csaCodigo, 
	            'nseCodigo' : <%=TextHelper.forJavaScript(CodedValues.NSE_PLANO_ODONTOLOGICO)%>
	        },
	        success: function (data) {
	        	var data = JSON.parse(data); 
	            var detalhesBeneficios = $('#detalhesBeneficiosPlanoOdontologico');
	            detalhesBeneficios.empty();
	            
	            for (var i = 0; i < data.beneficios.length; i++) {
	            	<%for (String benCodigo : benCodigos) {%>
	            		var beneficioCodigo = <%=benCodigo%>;
	            		if (beneficioCodigo == data.beneficios[i].id) {
	    	            	var tmp = htmlTagBase.replaceAll("<BEN_CODIGO>", data.beneficios[i].id);
	    	            	tmp = tmp.replaceAll("<BEN_DESCRICAO>", data.beneficios[i].detalhe);
	    	            	tmp = tmp.replaceAll("<ACTION_TR>", "preSimulaAjaxPlanoOdontologico(this)");
  ...
  function preSimulaAjaxPlanoOdontologico(x) {
	  x.querySelector(".form-check-input").checked = true;
	  simulaAjaxPlanoOdontologico()
  }
    
  $("#resultadoPlanoOdontologico .selecionarColuna,#resultadoPlanoOdontologico #checkAll,#resultadoPlanoOdontologico input[name='selecionarCheckBox'], a[name='selecionaAcaoSelecionar']").click(simulaAjaxPlanoOdontologico);
  ...
  function simulaAjaxPlanoOdontologico() {
	  ...
	  var benCodigo = $("#detalhesBeneficiosPlanoOdontologico :radio:checked").val()
	  var csaCodigo = $('#selectOperadoraPlanoOdontologico').find(':selected')[0].value;
	  if (typeof(benCodigo) != "undefined") {
		  showModal();
		  
		  $.ajax({
			    type: 'POST',
			    url: '../v3/reativarBeneficiarioSimulacaoBeneficios/simulaAjax?<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&_skip_history_',
			    data: {
  ...
  function trataErroDadosSimulacao(request, status, error) { 
	  	try {
			...
			if (request.status == 500) {
		        ...
		        $("#resultadoPlanoOdontologico input:checkbox[name=selecionarCheckBox]:checked").each(function(i, e){
		  			e.checked = false;
		  	  	});
				...
				$("#detalhesBeneficiosPlanoOdontologico :radio:checked").prop('checked', false);
				hideModal();
			} else if (request.status == 406) {
				$('.selecionarLinha').css("display","none");
				$("#detalhesBeneficiosPlanoOdontologico").html("");
				hideModal();
			} else if (request.status == 307) {
				postData('../v3/reativarBeneficiarioSimulacaoBeneficios?acao=planoOdontologico&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&RSE_CODIGO=<%=TextHelper.forJavaScript(rseCodigo)%>');
			} else { 
				postData('../v3/carregarPrincipal');
			}	
		} catch (err) {
  ...
  function preencherDadosSimulacao(data, pararModal) {
	  try {
	    ...
	    for (var i = 0; i < dadosNse.beneficiariosSemCalculos.length; i++) {
	      ...
	      $('#linha_plano_odontologico_' + dadosNse.beneficiariosSemCalculos[i].bfcCodigo).removeClass("table-checked");
	    }
  ...
  function proximo() { 
	  showModal();
	  postData('../v3/reativarBeneficiarioSimulacaoBeneficios?acao=detalhes&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&RSE_CODIGO=<%=TextHelper.forJavaScript(rseCodigo)%>');
  }
