...
%>
<c:set var="title">
  <hl:message key="rotulo.simulacao.beneficio.titulo.saude" />
</c:set>
      ...
      <div class="card">
        ...
        <div class="card-body table-responsive">
          <form>
            <div class="row">
              <div class="col-sm">
                <%=TextHelper.forHtmlContent(contratoBeneficio.getAttribute(Columns.CSA_NOME)) %>
              </div>
            </div>
            <div class="row">
              <div class="col-sm">
      ...
      <div class="card">
        ...
        <div class="card-body divTable">
          <div class="divTableBody text-center">
            ...
            <%} %>  
              <div class="divTableCell font-weight-bold" id="totalMensalidadePlanoSaude"><%=TextHelper.forHtmlAttribute(NumberHelper.format(valorTotal.doubleValue(), NumberHelper.getLang()))%></div>
              <div class="divTableCell font-weight-bold" id="totalSubsidioPlanoSaude"><%=TextHelper.forHtmlAttribute(NumberHelper.format(valorSubsidio.doubleValue(), NumberHelper.getLang()))%></div>
        ...
        <div class="card-body table-responsive ">
          <table class="table table-striped table-hover">
            ...
            <tbody>
              ...
              %>
              <tr class="<%=temContrato ? "table-checked" : "selecionarLinha"%>" id="linha_plano_saude_<%=TextHelper.forHtmlAttribute(beneficiario.getAttribute(Columns.BFC_CODIGO))%>">
                ...
                <td class="<%=!temContrato ? "selecionarColuna" : ""%>" id="mensalidade_plano_saude_<%=TextHelper.forHtmlAttribute(beneficiario.getAttribute(Columns.BFC_CODIGO))%>"><%=temContrato ? TextHelper.forHtmlAttribute(NumberHelper.format(total.doubleValue(), NumberHelper.getLang())) : ""%></td>
                <td class="<%=!temContrato ? "selecionarColuna" : ""%>" id="subsidio_plano_saude_<%=TextHelper.forHtmlAttribute(beneficiario.getAttribute(Columns.BFC_CODIGO))%>"><%=temContrato ? TextHelper.forHtmlAttribute(NumberHelper.format(subsidio.doubleValue(), NumberHelper.getLang())) : ""%></td>
                <td>
                  <% if (!temContrato) { %>
...
<%} else {%>
    <a class="btn btn-outline-danger" href="#no-back" onClick="postData('../v3/incluirBeneficiarioSimulacaoBeneficios?acao=iniciar&<%=SynchronizerToken.generateToken4URL(request)%>')" value="Cancelar"><hl:message key="rotulo.botao.voltar" /></a>
<%} %>
...
<c:set var="javascript">
  ...
  $(".selecionarColuna, a[name='selecionaAcaoSelecionar'], input[name='selecionarCheckBox'], #resultadoPlanoSaude #checkAll").click(simulaAjaxPlanoSaude);
  ...
  function proximo() { 
	  showModal();
	  postData('../v3/incluirBeneficiarioSimulacaoBeneficios?acao=planoOdontologico&reiniciarLocalSession=false&<%=TextHelper.forJavaScript(SynchronizerToken.generateToken4URL(request))%>&RSE_CODIGO=<%=TextHelper.forJavaScript(rseCodigo)%>&beneficiariosComContratoSaude=<%=TextHelper.forJavaScript(TextHelper.join(beneficiariosComContrato, ","))%>');
  }
