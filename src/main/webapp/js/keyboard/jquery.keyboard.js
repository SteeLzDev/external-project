...
;(function (factory) {
	...
	base.startup = function () {
		...
		//inicia o teclado em uma posição aleatória da tela
		if (document.getElementById('senha_keyboard') != null && document.getElementById('senha_keyboard') != 'undefined') {
			document.getElementById('senha_keyboard').style.top = Math.floor(Math.random() * ( (window.innerHeight * 0.35) - (window.innerHeight * 0.2))) + (window.innerHeight * 0.2);
			document.getElementById('senha_keyboard').style.left = Math.floor(Math.random() * ( (window.innerWidth * 0.35) - (window.innerWidth * 0.2))) + (window.innerWidth * 0.2);	
		}
	};
	...
	base.insertText = function (txt) {
		...
		//Faz o teclado mudar de posição a cada clique que não seja uma ação
		if (!isBksp) {
			document.getElementById('senha_keyboard').style.top = Math.floor(Math.random() * ( (window.innerHeight * 0.35) - (window.innerHeight * 0.2))) + (window.innerHeight * 0.2);
			document.getElementById('senha_keyboard').style.left = Math.floor(Math.random() * ( (window.innerWidth * 0.35) - (window.innerWidth * 0.2))) + (window.innerWidth * 0.2);
		}
			...
			if (/^\{\S+\}$/.test(keys[key])) {
				...
				case 'accept':
					base
						...
						.addClass(o.css.buttonAction + ' ' + kbcss.keyAction + ' botaokeyboardacao');
					break;
				...
				case 'shift':
					base
						.addKey('shift', action)
						.addClass(kbcss.keyHasActive + ' botaokeyboardacao');
					break;
