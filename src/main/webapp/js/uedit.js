...
var Uedit = function(){
    ...
    var HELP_URL = "../v3/exibirAjudaMarkdown?acao=ajudar";
    ...
    this.removeTag = function(tag, all){
        var str = this.getSelectedText();
    	var newStr = str
        if(!all) {
        	newStr = str.substring(tag.length,str.length-tag.length);
        	this.replaceSelectedText(newStr)
        	return newStr;
        }
        var re
        if (tag=="#") {
   	    ...
   	    while (newStr.substring(0,1) == tag) {
   		    newStr = newStr.replace(re, "$2");
        }
    	this.replaceSelectedText(newStr)
        return newStr;
    }
    ...
    this.createForm = function(width, height){
        ...
        form.closeButton.innerHTML = '<img src="../img/uedit/close_icon.png" />';
    ...
    this.comporHead = function(tag) {
    	var exp = "###"
        var head = ""
        var str = this.getSelectedText();
        	
    	for (i=0;i<exp.length;i++) {
    		var re = new RegExp(exp.substring(i), "g")
    	...
    	if (head.length>0 && str.substring(0,head.length) == head && str.substring(str.length-head.length) == head) {
    		str = this.removeTag(head, false);
    		if (str.substring(0,tag.length)==tag) {
                str = this.removeTag(tag, false);
            } else {
            	str = this.applyTag(tag, tag);
            }
    		str = this.applyTag(head)
    	} else {
    		if (str.substring(0,tag.length)==tag) {
                str = this.removeTag(tag, false);
            } else {
    ...
    this.button_colorYellow = function(){
        this.saveState();
        var sel = this.comporHead("++")
        this.purgeCachedSelection();
        return sel;
    ...
    this.button_colorGreen = function(){
        this.saveState();
        var sel = this.comporHead("+++")
        this.purgeCachedSelection();
        return sel;
    ...
    this.button_colorBlue = function(){
        this.saveState();
        var sel = this.comporHead("++++")
        this.purgeCachedSelection();
        return sel;   
    ...
    this.button_estiloTitulo = function(){
        this.saveState();
        var sel = this.comporHead("$!")
        this.purgeCachedSelection();
        return sel;   
    ...
    this.button_estiloSubtitulo = function(){
        this.saveState();
        var sel = this.comporHead("$@")
        this.purgeCachedSelection();
        return sel;   
    ...
    this.button_estiloTexto = function(){
        this.saveState();
        var sel = this.comporHead("$%")
        this.purgeCachedSelection();
        return sel;   
    ...
    this.button_link = function(){
        ...
        form.okButton.innerHTML = "Insert <img src='../img/uedit/done-icon.png' />";
    ...
    this.button_video = function(){
        var form = this.createForm(500, 76);
        
        form.urlLabel = document.createElement('p');
        form.urlLabel.style.left = "17px";
        form.urlLabel.style.top = "20px";
        form.urlLabel.className = "uedit_ui_form_label";
        form.urlLabel.innerHTML = mensagem('rotulo.markdown.video');
        
        form.urlBox = document.createElement('input');
        form.urlBox.className = "uedit_ui_form_text";
        form.urlBox.type = "text";
        form.urlBox.placeholder = mensagem('rotulo.markdown.video.placeholder');
        form.urlBox.style.right = "60px";
        form.urlBox.style.width = "300px";
        form.urlBox.style.top = "20px"
        
        form.okButton = document.createElement('a');
        form.okButton.className = "uedit_ui_form_button_ok";
        form.okButton.innerHTML = mensagem('rotulo.markdown.inserir') + " <img src='../img/uedit/done-icon.png' />";
        form.okButton.style.width = "500px";
        ...
        form.okButton.onclick = (function(){
            ...
            var vimeoMatch = id.match(vimeoRegex);
            if (vimeoMatch && vimeoMatch[1].length > 7) {
              id = vimeoMatch[1];
            }
            
            var videoTag = "increment_video[" + id + "]final";
            var linkvideo = mensagem('rotulo.markdown.video.link') + " increment_url_" + id;
            
            // Inject the tag
            this.saveState();
            this.injectText(videoTag);  
            this.injectText(linkvideo);
    ...
    this.button_image = function(){
        ...
        form.urlLabel.style.top = "10px";
        form.urlLabel.className = "uedit_ui_form_label";
        form.urlLabel.innerHTML = mensagem('rotulo.markdown.url.imagem.label');
        
        form.titleLabel = document.createElement('p');
        form.titleLabel.style.left = "10px";
        form.titleLabel.style.top = "30px";
        form.titleLabel.className = "uedit_ui_form_label";
        form.titleLabel.innerHTML = mensagem('rotulo.markdown.titulo.imagem');
        ...
        form.altLabel.innerHTML = mensagem('rotulo.markdown.texto.alt.imagem');
        
        form.urlBox = document.createElement('input');
        form.urlBox.className = "uedit_ui_form_text";
        form.urlBox.type = "text";
        form.urlBox.placeholder = mensagem('rotulo.markdown.url.imagem.exemplo');
        form.urlBox.style.right = "20px";
        ...
        form.titleBox.className = "uedit_ui_form_text";
        form.titleBox.type = "text";
        form.titleBox.placeholder = mensagem('rotulo.markdown.titulo.imagem.exemplo');
        form.titleBox.style.right = "20px";
        ...
        form.altBox.placeholder = mensagem('rotulo.markdown.texto.alt.imagem.exemplo');
